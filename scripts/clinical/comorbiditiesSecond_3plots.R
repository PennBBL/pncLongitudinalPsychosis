...
diag_df <-  diag_df[diag_df$timepoint == 't2', ]
...
td_baseline_plot <- ggplot(final_sum_df[final_sum_df$first_diagnosis == 'TD',],
  ...
  labs(title='TD - First Diagnosis: Comorbidities at Second Visit Related to Final Diagnosis') #+
...
op_baseline_plot <- ggplot(final_sum_df[final_sum_df$first_diagnosis == 'OP',],
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1), legend.position='bottom') +
  coord_cartesian(ylim=c(0, 100)) + labs(fill = 'Final Diagnosis') +
  labs(title='OP - First Diagnosis: Comorbidities at Second Visit Related to Final Diagnosis')
...
ps_baseline_plot <- ggplot(final_sum_df[final_sum_df$first_diagnosis == 'PS',],
  ...
  theme(axis.text.x=element_text(angle=45, hjust=1), legend.position='bottom') +
  coord_cartesian(ylim=c(0, 100)) + labs(fill = 'Final Diagnosis') +
  labs(title='PS - First Diagnosis: Comorbidities at Second Visit Related to Final Diagnosis')

pdf(file='~/Documents/pncLongitudinalPsychosis/plots/secondComorbid_3plots.pdf', width=21, height=7)
