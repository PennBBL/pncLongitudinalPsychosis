...
cog_df <- read.csv('~/Documents/pncLongitudinalPsychosis/data/cognitive/cnb_quickFA_impute_2020-10-20.csv')
...
mod0_rat <- glm(PS_final ~ ageAtClinicalAssess1 + first_diagnosis, family='binomial', data=final_df)
mod1_rat <- glm(PS_final ~ ageAtClinicalAssess1 + ratings_sum, family='binomial', data=final_df)
mod2_rat <- glm(PS_final ~ ageAtClinicalAssess1 + CompCog_EFF, family='binomial', data=final_df)
mod3_rat <- glm(PS_final ~ ageAtClinicalAssess1 + first_diagnosis + ratings_sum + CompCog_EFF,
  family='binomial', data=final_df)
mod4_rat <- glm(PS_final ~ ageAtClinicalAssess1 + first_diagnosis + ratings_sum + CompCog_EFF + first_diagnosis:ratings_sum + first_diagnosis:CompCog_EFF + ratings_sum:CompCog_EFF,
  family='binomial', data=final_df)
mod5_rat <- glm(PS_final ~ ageAtClinicalAssess1 + first_diagnosis*ratings_sum*CompCog_EFF,
  family='binomial', data=final_df)
