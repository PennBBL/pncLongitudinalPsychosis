...
###
### Ellyn Butler
### September 10, 2020 - September 21, 2020
...
summary_df$Time_Sex <- paste(summary_df$Timepoint, summary_df$Sex, sep='_')
summary_df$Time_Sex <- ordered(summary_df$Time_Sex, c('First_Male', 'First_Female',
  'Second_Male', 'Second_Female'))


cnb_plot <- ggplot(summary_df, aes(Test, Score, group=Time_Sex, colour=Time_Sex)) +
  theme_linedraw() +
	geom_line(data=summary_df, stat='identity', size=2) +
  facet_nested(first_diagnosis ~ last_diagnosis) + ylim(-.8, .8) +
	...
	scale_color_manual(values=c('blue', 'red', 'green', 'orange'), guide = guide_legend(nrow=1))


pdf(file='~/Documents/pncLongitudinalPsychosis/plots/cnb_t1t2.pdf', width=8, height=7)
cnb_plot
dev.off()
