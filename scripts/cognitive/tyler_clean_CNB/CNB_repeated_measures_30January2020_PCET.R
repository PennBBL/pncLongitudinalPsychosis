...
# raw basic models
gam1 <- gamm(pcet_acc2~Sex+s(Age,by=Sex),random=list(bblid=~1),data=x)
gam2 <- gamm(pcet_rtcr~Sex+s(Age,by=Sex),random=list(bblid=~1),data=x)
...
pdf("Repeated-Measures_Visuals_PCET.pdf",width=10,height=6)
...
visreg(lme(pcet_acc2~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid),xvar="cnbAgemonths",by="Sex",overlay=TRUE,xlab="Age (Years)",ylab="Accuracy")  # linear
visreg(lme(pcet_rtcr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid),xvar="cnbAgemonths",by="Sex",overlay=TRUE,xlab="Age (Years)",ylab="Response Time")  # linear
...
plot.new()
title("dependent variable = pcet_acc2 (raw Accuracy)")
grid.table(summary(lme(pcet_acc2~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))$tTable)
plot.new()
title("dependent variable = pcet_rtcr (mean raw response time)")
grid.table(summary(lme(pcet_rtcr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))$tTable)
plot.new()
...
plot.new()
title("dependent variable = pcet_acc2 (raw Accuracy)")
grid.table(summary(lme(pcet_acc2~(int+Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid,na.action=na.exclude))$tTable) # try including interval
plot.new()
title("dependent variable = pcet_rtcr (mean raw response time)")
grid.table(summary(lme(pcet_rtcr~(int+Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid,na.action=na.exclude))$tTable) # try including interval
...
ACC_apr <- scale(winsor(lm(pcet_acc2~Age+Age_Squared+Age_Cubed+TP+TP_Squared+TP_Cubed+int+int_sq+int_cub,data=x)$residuals,trim=0.005))
RT_apr <- scale(winsor(lm(pcet_rtcr~Age+Age_Squared+Age_Cubed+TP+TP_Squared+TP_Cubed+int+int_sq+int_cub,data=x)$residuals,trim=0.005))
...
TestRetest_for_pcet <- c("","Accuracy (age- & practice-regressed)","Response Time (age- & practice-regressed)","Accuracy (age-regressed)","Response Time (age-regressed)")

pdf("TestRetest_PCET.pdf",width=15,height=15)
grid.table(cbind(TestRetest_for_pcet,rbind(header,round(res,3))))
