...
# raw basic models
gam1 <- gamm(cpf_cr~Sex+s(Age,by=Sex),random=list(bblid=~1),data=x)
gam2 <- gamm(cpf_w_rtcr~Sex+s(Age,by=Sex),random=list(bblid=~1),data=x)
...
#summary(lme(cpf_cr~Age_Squared+(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))
#summary(lme(dprime~Age_Squared+(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))
#summary(lme(cpf_w_rtcr~Age_Squared+(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))
...
pdf("Repeated-Measures_Visuals_CPF.pdf",width=10,height=6)
...
visreg(lme(cpf_cr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid),xvar="cnbAgemonths",by="Sex",overlay=TRUE,xlab="Age (Years)",ylab="Accuracy")  # linear
visreg(lme(cpf_w_rtcr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid),xvar="cnbAgemonths",by="Sex",overlay=TRUE,xlab="Age (Years)",ylab="Response Time")  # linear
...
plot.new()
title("dependent variable = cpf_cr (raw Accuracy)")
grid.table(summary(lme(cpf_cr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))$tTable)
plot.new()
title("dependent variable = cpf_w_rtcr (mean raw response time)")
grid.table(summary(lme(cpf_w_rtcr~(Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid))$tTable)
plot.new()
...
plot.new()
title("dependent variable = cpf_cr (raw Accuracy)")
grid.table(summary(lme(cpf_cr~(int+Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid,na.action=na.exclude))$tTable) # try including interval
plot.new()
title("dependent variable = cpf_w_rtcr (mean raw response time)")
grid.table(summary(lme(cpf_w_rtcr~(int+Sex+cnbAgemonths)^2,data=x,random = ~ 1 | bblid,na.action=na.exclude))$tTable) # try including interval
...
ACC_apr <- scale(winsor(lm(cpf_cr~Age+Age_Squared+Age_Cubed+TP+TP_Squared+TP_Cubed+int+int_sq+int_cub,data=x)$residuals,trim=0.005))
RT_apr <- scale(winsor(lm(cpf_w_rtcr~Age+Age_Squared+Age_Cubed+TP+TP_Squared+TP_Cubed+int+int_sq+int_cub,data=x)$residuals,trim=0.005))
...
TestRetest_for_CPF <- c("","Accuracy (age- & practice-regressed)","Response Time (age- & practice-regressed)","Accuracy (age-regressed)","Response Time (age-regressed)")

pdf("TestRetest_CPF.pdf",width=15,height=15)
grid.table(cbind(TestRetest_for_CPF,rbind(header,round(res,3))))
